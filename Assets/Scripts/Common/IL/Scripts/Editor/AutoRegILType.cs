...
namespace game.IL
{
    ...
    static partial class AutoRegILType
    {
        ...
        static bool IsEditorType(System.Type type)
        {
            ...
            if (!string.IsNullOrEmpty(ns) && (ns.StartsWith("GUIEditor.") || ns.StartsWith("UnityEditor.")))
                return true;

            if (type.FullName.Contains("GUIEditor"))
                return true;
        ...
        const string file_path = "Assets/Scripts/Common/IL/Auto/ILRegType.cs";
#if USE_HOT
        [UnityEditor.MenuItem("IL/导出委托")]
#endif
        static void Build()
        {
        ...
        [UnityEditor.MenuItem("IL/清除自动生成脚本",false,21)]
        static void Clear()
        {
            var path = ResourcesPath.dataPath + file_path.Substring(0, file_path.LastIndexOf("/")).Substring(file_path.IndexOf("/"));
       ...
       // [UnityEditor.MenuItem("IL/TestBuildCode")]
